...
public interface BRRS_M_PD_Detail_Repo extends JpaRepository<BRRS_M_PD_Detail_Entity, String> {
    
	
	   @Query(value = "select * from BRRS_M_PD_DETAILTABLE where REPORT_DATE = ?1", nativeQuery = true)
	   List<BRRS_M_PD_Detail_Entity> getdatabydateList(Date reportdate);

	   @Query(value = "select * from BRRS_M_PD_DETAILTABLE where REPORT_DATE = ?1 offset ?2 rows fetch next ?3 rows only", nativeQuery = true)
	   List<BRRS_M_PD_Detail_Entity> getdatabydateList(Date reportdate, int offset, int limit);
	   ...
	   int getdatacount(Date reportdate);
		...
		List<BRRS_M_PD_Detail_Entity> GetDataByRowIdAndColumnId(String rowId,String ColumnId,Date reportdate);
}
